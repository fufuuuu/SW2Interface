...
    log.error(`请在 ${process.cwd() + '\\sw2interface\\sw2interface.json'} 中填写目标swaggerUrl`);
    return;
  }
...
      pathResult && Object.keys(pathResult).forEach((item) => {
        const fileName = options.splitApis ? `${item}.ts` : options.apiResultFileName;
        // prettier格式化api文本
        prettierFormat(pathResult[item]).then((prettierContent) => {
          writeInFile(files.apiResultPath, fileName, prettierContent);
        })
      })
      requestUrls && prettierFormat(requestUrls).then((prettierContent) => {
...
      });
      let resultFileName = options.resultFileName;
      // 如果开启全局interface，则文件名后缀改为.d.ts
      if (options.interfacesConfig && options.interfacesConfig.global && !resultFileName.endsWith('.d.ts')) {
...
            encoding: 'utf-8',
          });
          templateContent && (contentWithModified = updateNullableFields(templateContent, prettierContent));
        }
        // 转换后的数据写入目标文件
...
            writeInFile(fileConfigs.targetPath, options.targetFileName, componentsContent);
