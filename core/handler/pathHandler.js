...
  const {summary,apiName,inputParams,responseType,method,path,outerData,requestConfig,headers,queryParams,innerData,pathParams} = pathObj;
  const apiTemplate = fs.readFileSync(options.apisConfig.apiTemplatePath, {
    encoding: 'utf-8',
  });
  let str = ''
  // 路径是否以a.b格式结尾，即是否使用requestURL
  const urlEndingRegex = /([a-zA-Z0-9.]+)$/;
  const pathParamsStr = pathParams.join(',');
  if (apiTemplate.includes('method:') && apiTemplate.includes('url:')) {
    // 匹配 return至({之间的字符串
    const axiosRegex = /return\s+(.*?)\(/
    // 取出return的axios字段
    const axiosStr = apiTemplate.match(axiosRegex)[1] || 'axios.request'
    // 匹配 url的字符串
    const urlRegex = /url:\s*([^,]+)/;
    let urlStr = apiTemplate.match(urlRegex)[1] || ''
    // 路径是否使用requestURL
    if (urlStr && urlEndingRegex.test(urlStr)) {
      const urlStrStart = urlStr.replace(urlStr.match(urlEndingRegex)[1], '');
      // 若路径携带参数，则拼接箭头函数
      urlStr = `${urlStrStart}requestURL.${pathParamsStr?`${apiName}Url(${pathParamsStr})`:apiName+'Url'}`
    } else {
      urlStr = `\`${path}\``
    }
    const requestBody = [queryParams, innerData, headers].filter((item) => item).join(', ');
...
    // 取出括号中的数据
    const urlRegex = /\(.*?\)/g;
    const urlParamsStr = apiTemplate.match(urlRegex)[1]
    // 匹配是否以(a.b)格式即是否使用requestURL
...
  if (options.apisConfig && options.apisConfig.apiTemplatePath && !fs.existsSync(options.apisConfig.apiTemplatePath)) {
...
    const apiObjs = paths[url];
    // 'get' | 'post' | 'delete' 请求方式数组
...
      const apiName = apiObj.operationId.replace(/Using\w*/, '');
      // 请求header处理
...
      if (apiParams) {
        // GET请求 query参数
        inputParams += (inputParams ? ', ' : '') + handleParameters(apiParams, outputFileName);
      }
      // 请求接口
...
        // 删除入参字符串中的空格并分割成数组
        const requestUrlInputParams = inputParams.replace(/\s/g, '').split(',')
        // 取出接口地址参数组成数组
        pathParams = requestUrlPath.match(/\$\{([^\}]+)\}/g).map(item => item.slice(2, -1));

        // 根据接口地址中的参数过滤接口入参
        const requestUrlParams = requestUrlInputParams.filter(item => pathParams.includes(item.split(':')[0]));
        // 接口地址函数
        requestUrlPath = `(${requestUrlParams.join(',')})=>\`${formatPath}\``
      }
      requestUrls[apiName+'Url'] = requestUrlPath;
      // 生成请求参数
      const queryParams = generateQueryParams(apiParams);
      // 生成请求配置
      const requestConfig = generateRequestConfig(queryParams, innerData, headers);
      let expression = '';
...
  });
  // 生成请求地址
  let requestUrlsStr = transformString(JSON.stringify(requestUrls))
  // 请求地址换行
...
  let result = [queryParams, innerData, headers].filter((item) => item).join(', ');
...
  if (schema.enum) {
    // 枚举类型首字母大写
    propertyName = propertyName.slice(0, 1).toUpperCase() + propertyName.slice(1) + '';
